...
describe 'A user' do
  ...
  it "does not require a password when updating" do
    ...
    user.update(name:"Updated User Name", password:"")
    expect(user.valid?).to eq(true)
...
describe "authenticate" do
  ...
  it "returns non-true value if the email does not match" do
    expect(User.authenticate("nomatch-email",@user.password)).not_to eq(true)
  end
  it "returns non-true value if the password does not match" do
    expect(User.authenticate(@user.email,"nomatch-password")).not_to eq(true)
  end
